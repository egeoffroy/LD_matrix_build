...
phenos <- 'Total_cholesterol' # make this into a parameter for pipeline
chrs <- c(1:22)
...
for(pop in 1:length(pops)){ #read in pop's .frq file for MAF
  ...
  for(pheno in phenos){ #read in GWAS output file
    GWAS_result <- fread(paste("/home/egeoffroy/Wojcik/Wojcik_build38/WojcikG_", pheno, ".txt.gz", sep = '') , header = T)
    ...
    for(chr in chrs){ # it may be more useful to also have chromosome as a parameter for the overall pipeline
      mpqtl <- fread("/home/egeoffroy/LD_matrix/cis_eQTLs_" %&% pops[pop] %&% "_all_cis_unpruned_fixed_build_fixed_window.txt", nThread = 40) #read in matrix eQTL results
    ...
    fwrite(pQTL_write, "/home/egeoffroy/coloc/pQTL_" %&% pops[pop] %&% "_" %&% pheno %&% ".txt", quote = F, sep = "\t", na = "NA", row.names = F, col.names = T)
    gzip("/home/egeoffroy/coloc/pQTL_" %&% pops[pop] %&% "_" %&% pheno %&% ".txt", destname = "/home/egeoffroy/coloc/pQTL_" %&% pops[pop] %&% "_" %&% pheno %&% ".txt.gz") #script may only take .gz values so can't hurt to be too careful
    fwrite(GWAS_write, "/home/egeoffroy/coloc/GWAS_TOPMED" %&% pops[pop] %&% "_" %&% pheno %&% ".txt", row.names = F, col.names = T, sep = "\t", quote = F, na = "NA")
    gzip("/home/egeoffroy/coloc/GWAS_TOPMED" %&% pops[pop] %&% "_" %&% pheno %&% ".txt", "/home/egeoffroy/coloc/GWAS_TOPMED" %&% pops[pop] %&% "_" %&% pheno %&% ".txt.gz")
