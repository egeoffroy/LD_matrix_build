...
##  gt_df <- gt_df[,which((effect_allele_freqs >= maf) | (effect_allele_freqs <= 1 - maf))]
  gt_df
} #is this what i should edit to find driving snp? so far, i'm thinking no. do it separately
...
#  cis_gt
#  column_labels <- paste('chr', column_labels, sep = '')
#  column_labels
#  print(column_labels)
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
                 covariates_file, chrom, pop, prefix, maf=0.01, n_folds=10, n_train_test_folds=5,
                 seed=NA, cis_window=10000, alpha=0.5, null_testing=FALSE,protein_file) {
  ...
  for (i in 1:n_genes) {
    ...
    print(cis_gt)
    if(length(cis_gt) > 2){
    	write.table(as.data.frame(cis_gt), paste('/home/egeoffroy/LD_matrix/10kb_of_gene/', pop, '_chr_', chrom, '_', gene, '_10kb_of_gene.txt', sep = ''), quote = F, row.names=F)
    }
  }
}
